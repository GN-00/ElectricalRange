...
namespace ProjectsNow.Views.FinanceView
{
    internal class ReceiptViewModel : ViewModelBase
    {
        ...
        public ReceiptViewModel(AccountTransaction transaction, ObservableCollection<AccountTransaction> transactions)
        {
            GetData();
        ...
        private void Save()
        {
            if (NewData.Id == 0)
            {
                string query = $"Select IsNull(Max(Number), 0) " +
                               ...
                               $"Where Type = '{Enums.AccountingTransactions.Receipt}'";

                using (SqlConnection connection = new(Database.ConnectionString))
                {
                    NewData.Number = connection.QueryFirstOrDefault<int>(query) + 1;
                    NewData.Code = $"REC{NewData.Number:000000}";
