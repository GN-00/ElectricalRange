...
namespace ProjectsNow.Views.FinanceView
{
    public class ReceiptsViewModel : ViewModelBase
    {
        ...
        public ReceiptsViewModel(IView view)
        {
            ViewData = view;
        ...
        private void GetData()
        {
            ...
            query = $"Select * From [Finance].[Receipts(View)] " +
                    $"Order By Date Desc ";
        ...
        private void New()
        {
            ...
            {
                Type = AccountingTransactions.Receipt.ToString()
            };

            Navigation.OpenPopup(new ReceiptView(transaction, Items), PlacementMode.Center, false);
        }
        private bool CanAccessNew()
        {
            if (!UserData.ModifyReceipts)
                return false;
        ...
        {
            ReceiptForm receiptForm = new(transaction);
        ...
        private void Export()
        {
            try
            {
                ...
                if (list.Count != 0)
                {
                    ...
                    string worksheetName = $"Receipts {DateTime.Now.Date:dd-MM-yyyy}";
