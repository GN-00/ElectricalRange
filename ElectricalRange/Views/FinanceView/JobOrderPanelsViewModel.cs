...
{
    public class JobOrderPanelsViewModel : ViewModelBase
    {
        ...
        public JobOrderPanelsViewModel(ObservableCollection<InvoiceItem> transactions, ObservableCollection<JPanel> panels)
        {
            TransactionsData = transactions;
            Items = panels;

            var panelCollection = ItemsCollection.Cast<JPanel>();
            if (panelCollection.Count() != 0)
            {
                SelectedItem = panelCollection.First();
            }
        ...
        private bool DataFilter(object panel)
        {
            ...
            if (((JPanel)panel).IsPercentageInvoice == true)
                return false;
        ...
        private void Add(JPanel panel)
        {
            ...
            if (Invoicing.Value < panel.ReadyToInvoicedQty)
            {
                Invoicing++;
            }
        }
        ...
        private void Post(JPanel panel)
        {
            ...
            panel.InvoicedQty += Invoicing.Value;
            int qty = Invoicing.Value;
            InvoiceItem newPanel = new()
            {
                ...
                Qty = Invoicing.Value,
                NetPrice = (double)panel.PanelEstimatedPrice * qty,
                UnitNetPrice = (double)panel.PanelEstimatedPrice,
                VATValue = (double)panel.PanelVATValue * qty,
                UnitVATValue = (double)panel.PanelVATValue,
                GrossPrice = (double)panel.PanelFinalPrice * qty,
